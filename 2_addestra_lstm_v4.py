...
# Iperparametri (stessi del v2 82%)
FINESTRA = 30
STRIDE = 15

# FEATURES ESTESE (per frame):
#   34 coordinate + 34 velocità + 1 distanza               = 69 (come prima)
#   + 1 delta_distanza (variazione distanza frame-to-frame)
#   + 4 centro di massa (cx, cy, vel_cx, vel_cy)
#   TOTALE: 74
NUM_FEATURES = 74
...
def crea_sequenze_da_csv(percorso_csv, categoria, filtrare):
    """
    Costruisce le sequenze con 74 features per frame:
      - 34 coordinate normalizzate dei 17 keypoints
      - 34 velocità (differenza temporale delle coordinate)
      - 1  distanza dal vicino più prossimo
      - 1  delta_distanza (variazione della distanza tra frame)
      - 2  centro di massa (cx, cy)
      - 2  velocità del centro di massa (vel_cx, vel_cy)
    """
    df = pd.read_csv(percorso_csv)
    sequenze_video = []

    # Centro di massa di ogni persona per ogni frame
    centri_per_frame = {}
    ...
    # Filtro spettatori (solo su fight in training)
    ...
    for id_persona, dati_persona in df.groupby('id_persona'):
        ...
        # --- Feature 3 (NUOVA): Delta distanza (1 valore) ---
        # Negativo = le persone si avvicinano (segnale di pericolo)
        # Positivo = le persone si allontanano
        delta_dist = np.diff(distanze, axis=0)
        delta_dist = np.vstack([np.zeros((1, 1)), delta_dist])

        # --- Feature 4 (NUOVA): Centro di massa + sua velocità (4 valori) ---
        centri_persona = []
        for frame_num in lista_frame:
            centri = centri_per_frame.get(frame_num, {})
            if id_persona in centri:
                centri_persona.append(centri[id_persona])
            else:
                # Fallback: calcola dal keypoint corrente
                idx = np.where(lista_frame == frame_num)[0][0]
                coords = coordinate[idx]
                cx = np.mean(coords[0::2])
                cy = np.mean(coords[1::2])
                centri_persona.append(np.array([cx, cy]))

        centri_persona = np.array(centri_persona)  # (T, 2) = [cx, cy]

        # Velocità del centro di massa
        vel_centro = np.diff(centri_persona, axis=0)
        vel_centro = np.vstack([np.zeros((1, 2)), vel_centro])  # (T, 2) = [vel_cx, vel_cy]

        # --- Concatena tutto: 34 + 34 + 1 + 1 + 2 + 2 = 74 features ---
        features = np.hstack([
            coordinate,       # 34: posizioni keypoints
            velocita,         # 34: velocità keypoints
            distanze,         # 1:  distanza dal vicino
            delta_dist,       # 1:  variazione distanza (NUOVA)
            centri_persona,   # 2:  centro di massa cx, cy (NUOVA)
            vel_centro,       # 2:  velocità centro massa (NUOVA)
        ])
...
class BiLSTMClassificatore(nn.Module):
    """
    Architettura BiLSTM: vede la sequenza sia in avanti che all'indietro.
    Il primo LSTM è bidirezionale (output = 128*2 = 256).
    Il secondo LSTM è bidirezionale (output = 64*2 = 128).
    I layer densi restano uguali.
    """
    def __init__(self):
        super(BiLSTMClassificatore, self).__init__()
        self.lstm1 = nn.LSTM(
            input_size=NUM_FEATURES,
            hidden_size=HIDDEN_1,
            batch_first=True,
            bidirectional=True    # ← NUOVA: bidirezionale
        )
        self.dropout1 = nn.Dropout(DROPOUT)
        self.lstm2 = nn.LSTM(
            input_size=HIDDEN_1 * 2,   # 128*2 = 256 (bidirezionale)
            hidden_size=HIDDEN_2,
            batch_first=True,
            bidirectional=True    # ← NUOVA: bidirezionale
        )
        self.dropout2 = nn.Dropout(DROPOUT)
        self.fc1 = nn.Linear(HIDDEN_2 * 2, DENSE_1)   # 64*2 = 128
    ...
    def forward(self, x):
        ...
        x, _ = self.lstm2(x)
        x = self.dropout2(x)
        x = x[:, -1, :]   # Ultimo timestep (contiene info da entrambe le direzioni)
...
def main():
    # Training: CON filtro spettatori
    print("Caricamento Train Set (con filtro spettatori)...")
    X_train, y_train = carica_dataset("train", filtrare=True)

    # Validation: CON filtro (stesso set del v2 per confronto equo)
    print("\nCaricamento Validation Set...")
    X_val, y_val = carica_dataset("val", filtrare=True)
    ...
    modello = BiLSTMClassificatore().to(dispositivo)
    ...
    print(f"\n{'='*60}")
    print(f" Inizio addestramento BiLSTM — max {EPOCHE} epoche")
    print(f"{'='*60}\n")
